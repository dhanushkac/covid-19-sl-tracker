...
import TreatmentCount from "../TreatmentCount";
...
const HospitalGrid = ({ data }) => {
  ...
  return (
    <Col xs={{ span: 24 }} sm={{ span: 24 }} md={{ span: 24 }} lg={{ span: 24 }} xl={{ span: 20 }}
      ...
      <Row gutter={[32, 16]}>
        {displayData && displayData.map(item => {
          ...
          return <Col key={item.hospital_id} xs={{ span: 24 }} sm={{ span: 12 }} md={{ span: 8 }} lg={{ span: 8 }}
                      xl={{ span: 6 }}
