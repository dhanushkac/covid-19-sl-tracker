...
function App() {
        ...
        .then(val => {
          ...
          const processedChartData = chartData.map(obj => {
            ...
            if (
              obj.recovered >= 2 &&
              obj.date < date &&
              obj.recovered > data.local_recovered
            ) {
  ...
  const cases = {
    ...
    newText: isLocal
      ? getStatusText(state.local_new_cases, NEW_CASES)
      : getStatusText(state.global_new_cases, NEW_CASES),
    icon: faAmbulance,
    style: "#ff4d4f"
  };
  ...
  const deaths = {
    ...
    newText: isLocal
      ? getStatusText(state.local_new_deaths, NEW_DEATHS)
      : getStatusText(state.global_new_deaths, NEW_DEATHS),
    icon: faProcedures,
    style: "#ff8f2f"
  };
    ...
    <Layout className="layout">
              ...
              <div>
                <Row className="card-list">
                  <CardPanel
                    cardData={data}
                    onChange={onChange}
                    lastUpdate={state.update_date_time}
                  />
                  <QAPanel />
                </Row>
                <Row>
      ...
      {!isLoading && (
        <Footer className="footer">
          ...
          Powered by{" "}
          <a href="http://hpb.health.gov.lk/api-documentation">
            hpb.health.gov.lk
          </a>
        </Footer>
      )}
